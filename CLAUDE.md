...
- `db.py` maintains a **SQLite mirror** of the metadata (`images`, `classifier_scores` tables) for fast querying without walking. The DB is rebuilt/refreshed by scanning the filesystem; treat it as a cache, not authoritative state. `db_api.py` exposes query helpers; `db_view_webapp.py` is a legacy Flask UI on top of it. The production website lives in the `nightskycam-services` repo (`nightskycam-website` package), consuming this package's public API (`nightskycam_images/__init__.py`).
...
- `ns.files.*` — backup, deletion, move-clear, remove-selected, `move-list` (`LIST_FILE ROOT DESTINATION [--second-root R2] [--dry-run]` — move the images named in a list file, one filename_stem per line as emitted by `ns.db.filter`, together with their `.toml` and thumbnail, from whichever root holds each into DESTINATION keeping the `system/date` structure (resolves after-midnight frames via the observing-night folder); an image already present & byte-identical in DESTINATION has its redundant source deleted (so it still leaves the dataset), while a target that exists but DIFFERS is a logged conflict left wholly in place (never overwritten); the run continues past both; does not touch the DB — run `ns.db.update` after), `stats` (filesystem-walk report) (and `ns.files.web.*` for file-based webapps)
